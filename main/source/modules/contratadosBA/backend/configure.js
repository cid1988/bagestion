exports = module.exports = function(app, conf) {
    require('express-csv');
    app.get('/api/contratadosBA-csv/download', function(req, res) {
        ...
        require('../../../db.js').getDbInstance(function(err, db) {
            ...
            db.collection('contratadosBA').find({}).each(function(err, item) {
                if (err) {
                    res.status(503);
                    console.log(err);
                    return res.end();
                ...
                if (item) {
                    ...
                    res.setHeader("Content-Disposition", "attachment; filename=\"contratadosBA.csv\"");
                    res.csv(all);
                }
            });
        });
