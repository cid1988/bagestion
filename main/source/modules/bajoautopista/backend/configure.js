exports = module.exports = function(app, conf) {
   ...
   var doAll = function(db, res, nombresComunas, nombresJurisdicciones) {
        ...
        var all = [
            ["Comuna", "ID Mapa", "Predio", "Autopista", "Desde", "Hasta", "Dirección Exacta", "Usuario", "Uso", "Destino", "Instrumento", "Ocupación", "Status", "Fecha Inicio", "Fecha Fin", "Canon", "Valor Mensual ($)", "Uso Especifico", "Plan de Acción", "Fecha Seguimiento", "Responsable", "Sección", "Manzana", "Parcelas", "Superficie Total", "Superficie Cubierta", "FOT", "Fecha Ultima Observacion", "Ultima Observacion"]
        ];
        db.collection('bajoautopista').find({}).each(function(err, item) {
            ...
            if (item) {
                ...
                item.entre1,
                item.entre2,
                item.calleExacta && item.calleExacta + ' ' + item.numeroExacto,
                item.usuario,
                item.uso,
                item.destino,
                item.instrumento,
                ...
                item.fot,
                item.comentarios && item.comentarios[item.comentarios.length-1] && item.comentarios[item.comentarios.length-1].cuando,
                item.comentarios && item.comentarios[item.comentarios.length-1] && item.comentarios[item.comentarios.length-1].descripcion.replace("\n"," ")]);
            }
            else {
