...
//Detalle de contactos---------------------------------------------------------------------------------
    ...
    $scope.agregarTelefonos = function(dataTelefonos) {
        if (!$scope.contacto.telefonos) {
            $scope.contacto.telefonos = [];
        }
         if ($scope.dataTelefonos.valor.match(/[a-z]/ig)){
            $scope.letras = true;
            $scope.dataTelefonos.valor = '';
            
         } else {
               $scope.letras = false;
               $scope.dataTelefonos.valor = $scope.dataTelefonos.valor.replace(/-/g,"");
               
               //push y vacío los campos
    ...
    //PRUEBA agregar correo
    $scope.correoNuevo = function(correoPrueba) {
       if(!$scope.contacto.correos){
          
       }
       else{
          $scope.contacto.correos.push($scope.correoPrueba);
          $scope.correoPrueba = {
               nombre: ''
...
//Nuevo contacto---------------------------------------------------------------------------------------------
   ...
   $scope.setAltura=function()
   {
    ...
    $scope.agregarTelefonos = function(dataTelefonos) {
        ...
        //si tiene letras no voy a poder guardar, medio turbio
        if ($scope.dataTelefonos.valor.match(/[a-z]/ig)){   
          $scope.letras = true;
          $scope.dataTelefonos.valor = '';
