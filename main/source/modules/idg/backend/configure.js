exports = module.exports = function(app, conf) {
    require('express-csv');
    app.get('/api/idg-csv/download', function(req, res) {
        ...
        require('../../../db.js').getDbInstance(function(err, db) {
            ...
            db.collection('idg').find({}).each(function(err, item) {
                if (err) {
                    res.status(503);
                    console.log(err);
                    return res.end();
                ...
                if (item) {
                    ...
                    res.setHeader("Content-Disposition", "attachment; filename=\"idg.csv\"");
                    res.csv(all);
                }
            });
        });
