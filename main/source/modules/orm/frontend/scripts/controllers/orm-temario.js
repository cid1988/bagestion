...
.controller('ORMTemarioNavbarCtrl', function($scope, $rootScope) {
...
}).controller('ORMTemarioCtrl', function($rootScope, $scope, ORMColoresPorTipo, ORMContacto, $routeParams, ORMTemario, ORMInstanciaReunion, ORMTema, ORMReunion, $modal, $window) {
    $scope.cancelacion = "";
    ...
    $scope.contactoPorIdParaEnvio=function(id)
    {
        $scope.contactos = ORMContacto.list(function()
        {
            for (var i = 0; i < $scope.contactos.length; i++) {
                  if ($scope.contactos[i]._id == id)
                  {
                      $scope.textoMail=$scope.textoMail+"<b>"+$scope.contactos[i].apellidos+" "+$scope.contactos[i].nombre+"</b><br/>";
                       $scope.textoMail=$scope.textoMail+"<b>DG Coordinación de Gestión</b><br/>"+
    ...
    $scope.armarMail=function()
    {
        $scope.textoMail="<div class='mailwrapper' style='font-size:17px; color:black; line-height:100% !important;'>"+
        "Estimados,<br/><br/>"+
        "A continuación les enviamos la propuesta de temario para la reunión de referencia, a realizarse el "+
        "<b><u>"+$scope.dias[$scope.fecha.getDay()]+" "+$scope.fecha.getDate()+" "+$scope.meses[$scope.fecha.getMonth()]+" a las "+$scope.hora+'hs ('+($scope.instancia.hastaDate-$scope.instancia.desdeDate)/60000+" min)</u></b>";
        if ($scope.instancia.ubicacion) {
            $scope.textoMail = $scope.textoMail + '<b><u> en ' + $scope.instancia.ubicacion.nombre + '.</b></u>';
        }
        
        $scope.textoMail=$scope.textoMail+"<br/><br/>Disponen de 48 hs hábiles para incorporar sugerencias o propuestas al mismo.<br/>";
        /*$scope.textoMail=$scope.textoMail+"<br/><br/>Disponen hasta el miércoles 10 de Febrero a las 12:00hs para incorporar sugerencias o propuestas al mismo.<br/><br/>"+
        "No obstante, teniendo en cuenta que nos encontramos en pleno proceso de cierre de la Planificación 2016, tengan en cuenta que este temario podría llegar a sufrir modificaciones por expreso pedido de Jefatura de Gobierno.<br/><br/>";*/
        
        if($scope.temario && $scope.temario.html)
        {
            $scope.textoMail=$scope.textoMail+
                "<div style='font-family: monospace; color: #081367;font-size: 1em; padding-left:20px;padding-bottom:20px;border-top: 1px solid #081367 !important;border-bottom: 1px solid #081367 !important;'>"+
                    "<b>"+
                        "<br/>"+
                        "<u>PROPUESTA DE TEMARIO:</u>"+
                        "<br/><br/><br/>"+
                        "<div>"+
                            $scope.temario.html+
                        "</div>"+
        ...
        $scope.textoMail=$scope.textoMail+"<br/>Quedamos a su disposición ante cualquier consulta.<br/><br/>"+
            "Saludos Cordiales.<br/><br/>";

        if($scope.reunion && $scope.reunion.asistenteTablero)
        {   
            $scope.contactoPorIdParaEnvio($scope.reunion.asistenteTablero);
        }
        else
        {
            $scope.textoMail=$scope.textoMail+"<b>temarios@buenosaires.gob.ar</b><br/>"+
            "<b>DG Coordinación de Gestión</b><br/>"+
            "<b>Secretaría de Planeamiento y Control de Gestión</b><br/>"+
            "<b>Jefatura de Gabinete de Ministros</b><br/>"+
            "<b>Tel: + 54 11 5091-7200 Int: 7215</b></div>";
        }
    };
...
.controller('ORMTemarioListaEnvioCtrl', function($scope, $state, $routeParams, ORMInstanciaReunion, ORMTemario, ORMReunion, $rootScope, ORMContacto, $http) {
    ...
    if($scope.temario){
        var idTemario=$scope.temario._id;
    }
    ...
    $scope.siNoExiste=function(valor, padre){
        if(valor && !padre)
        {
            return valor;
        }
    ...
    /*$scope.temarioEnvio=ORMTemario.get({instancia:$scope.instancia._id})*/
    $scope.enviar = function() {
        ...
        if ($scope.temario.tipoTemario == "propuesta") {
            if ($scope.instancia.reunion == '531e45b0ee7b07a673df4e68') {
                var payload = {
                    ...
                    temarioId: idTemario,//$scope.temario._id,
                    principioHtml: "<div style='font-size: 20px; font-family: Arial; line-height: 120%;padding-top: 15px;padding-left: 5px;border-right-width: 10px;padding-right: 5px;padding-bottom: 10px;background-color: #FFD300;text-align: center;'>"+
                            "<b>Propuesta de " + $scope.asunto + "</b>"+
                        "</div><br/><hr/>"+
                        $scope.textoMail+"<br/><br/>",
                    finHtml: "",
                    adjunto : adjunto,
                ...
                var payload = {
                    asunto: $scope.cancelacion + "Propuesta de " + $scope.asunto,
                    para: $scope.reunion.propuestaTemario.para
                        .concat($scope.maestro.propuestaTemario.para)
                            .concat(mails("propuestaTemario","para")),
                    cc: $scope.reunion.propuestaTemario.cc
                        .concat($scope.maestro.propuestaTemario.cc)
                            .concat(mails("propuestaTemario","cc")),
                    cco: $scope.reunion.propuestaTemario.cco
                        .concat($scope.maestro.propuestaTemario.cco)
                            .concat(mails("propuestaTemario","cco")),
                    exclusivos: $scope.siNoExiste($scope.reunion.propuestaTemario.exclusivos)
                        .concat($scope.siNoExiste($scope.maestro.propuestaTemario, "exclusivos").exclusivos),
                    temarioId: idTemario,//$scope.temario._id,
                    principioHtml: "<div style='font-size: 20px; font-family: Arial; line-height: 120%;padding-top: 15px;padding-left: 5px;border-right-width: 10px;padding-right: 5px;padding-bottom: 10px;background-color: #FFD300;text-align: center;'>"+
                            "<b>Propuesta de " + $scope.asunto + "</b>"+
                        "</div><br/><hr/>"+
